...
function renderBoard(board) {
    ...
    for (var i = 0; i < board.length; i++) {
        ...
        for (var j = 0; j < board[0].length; j++) {
            ...
            var backgroundClor = gBoard[i][j].isMine ? gBoard[i][j].isShown ? !gBoard[i][j].isMarked ? 'beige' : '' : '' : ''
...
function cellClicked(cell, i, j) {
    ...
    gNewMatCopy = copyMat(gBoard)
    gPrevBoard.push(gNewMatCopy)
    elUndoClick.innerText = gPrevBoard.length > 0 ? `${gPrevBoard.length} more clicks left` : ''
...
function getrendHtml(cell) {
    ...
    if (cell.isMine && cell.isShown && !cell.isMarked) return `<img class="mine" src="img/explosion.gif" alt="explosion">`
    if (cell.isMarked && cell.isShown) return `<img class="flag flag-hide" src="img/red-flag.gif" alt="close">`
...
function undoBtn() {
    if (gPrevBoard.length > 0) {
        elUndoClick.innerText = gPrevBoard.length > 0 ? `${gPrevBoard.length - 1} more clicks left` : ''
